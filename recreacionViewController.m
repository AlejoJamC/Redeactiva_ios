...
#import "recreacionViewController.h"
#import "recreacionTableViewCell.h"

@interface recreacionViewController ()

@end

@implementation recreacionViewController
...
- (void) viewDidAppear:(BOOL)animated {
    ...
    [self cargarRecreativas];
    
}

-(void)cargarRecreativas{
    _cargando = true;
    NSString *url =
    [NSString stringWithFormat:@"%@/containers/getcontainers/proyectos/%i",app().urlServicio, _currentPage];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.filasArray.count != 0) {
        if(indexPath.row <[self.filasArray count]){
            NSDictionary *proyecto = [self.filasArray objectAtIndex:indexPath.row];
            
            [cell.titulo setText:[proyecto valueForKeyPath:@"nombreprograma"]];
            
            NSString *url = [proyecto valueForKeyPath:@"logo"];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    if (!_cargando && !_noMoreResultsAvail) {
        ...
        {
            [self performSelector:@selector(cargarRecreativas) withObject:nil afterDelay:1];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.row < [self.filasArray count]){
        ...
        [rutaSelect setValue:[rutaSelect objectForKey:@"paginaweb"] forKey:@"reglamento"];
        [app() setRuta:rutaSelect];
        
        
        [self.navigationController pushViewController:[self.storyboard instantiateViewControllerWithIdentifier:@"recreacion"] animated:YES];
    }
}
